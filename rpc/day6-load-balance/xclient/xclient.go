...
// XClient 的构造函数需要传入三个参数，
// 服务发现实例 Discovery、负载均衡模式 SelectMode 以及协议选项 Option。
// 为了尽量地复用已经创建好的 Socket 连接，
// 使用 clients 保存创建成功的 Client 实例，
// 并提供 Close 方法在结束后，关闭已经建立的连接。
...
func (xc *XClient) Broadcast(ctx context.Context, serviceMethod string, args, reply interface{}) error {
	...
	for _, rpcAddr := range servers {
		...
		go func(rpcAddr string) {
			...
			if err != nil && e == nil {
				e = err
				cancel() // 只要有一个 call 错误，剩余未完成的 call 都取消不再执行
			}
