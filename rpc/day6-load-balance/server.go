...
var DefaultOption = &Option{
	...
	ConnectTimeout: 10 * time.Second, // 连接10s超时
}
...
func (server *Server) handleRequest(cc codec.Codec, req *request, sending *sync.Mutex, wg *sync.WaitGroup, timeout time.Duration) {
	...
	// 0 表示没有限制，等待 called 和 sent 执行完毕
	if timeout == 0 {
		<-called
		<-sent
		return
...
// ServeHTTP 方法实现了 http.Handler 接口用来应答 RPC 请求
func (server *Server) ServeHTTP(w http.ResponseWriter, req *http.Request) {
	...
	// Hijack 用来接管 http 请求
	// 通过 Hijack() 方法，调用者可以获取到底层的 TCP 连接，并且负责连接的管理和关闭
...
// HandleHTTP 为 RPC 消息在 rpcPath 上注册一个 HTTP 处理器
// 用来接管 http.Serve() 函数在调用的时候的 HTTP 请求
//
